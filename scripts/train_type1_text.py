...
DATA_DIR = BASE_DIR / "data" / "refined" / "type1_text"
OUTPUT_DIR = BASE_DIR / "models" / "type1_text"
...
class PerplexityCallback(TrainerCallback):
    """Evaluate perplexity on validation set (skips reasoning blocks)"""
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % self.eval_steps == 0 and state.global_step > 0:
            ppl = self._evaluate(state.global_step)
            ...
            if ppl <= self.target:
                print(f"\nTarget perplexity {self.target} reached!")
                control.should_training_stop = True

    def _evaluate(self, step: int) -> float:
        print(f"\n{'='*50}")
        print(f"Type 1 (TEXT) Evaluation - Step {step}")
        print(f"{'='*50}")

        self.model.eval()
        total_loss = 0
        count = 0
        had_reasoning = 0  # Track how many had reasoning blocks (skipped)
        ...
        with torch.no_grad():
            for sample in samples:
                try:
                    ...
                    # Check if had reasoning block (for stats)
                    if R_START in response or R_END in response:
                        had_reasoning += 1

                    # Skip reasoning block if present, evaluate only the actual answer
                    clean_response = skip_reasoning_block(response)

                    # Calculate loss on the expected text
                    text = sample['text']
        ...
        reasoning_pct = 100 * had_reasoning / count if count > 0 else 0
...
def main():
    ...
    print("=" * 60)
    print("Type 1: TEXT Training (No Reasoning)")
    print("=" * 60)
    ...
    callback = PerplexityCallback(eval_data, tokenizer, model,
