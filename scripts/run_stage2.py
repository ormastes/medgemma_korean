#!/usr/bin/env python3
"""
Stage 2: Train NEW output embeddings only
This script trains only the new Korean output embeddings (lm_head) while keeping
...
# Directories
STAGE1_MODEL_DIR = "models/staged_training/stage1_new_input_embeds"
DATA_DIR = "data/processed"
OUTPUT_DIR = "models/staged_training/stage2_new_output_embeds"

os.makedirs(OUTPUT_DIR, exist_ok=True)

print(f"Input model: {STAGE1_MODEL_DIR}")
print(f"Output dir: {OUTPUT_DIR}")

# Load token mapping
mapping_path = f"{STAGE1_MODEL_DIR}/token_mapping.json"
...
# Enable only output embeddings (lm_head)
...
# Create hook to freeze OLD output embeddings during training
...
freeze_hook = FreezeOldEmbeddingsHook(actual_original_vocab)
hook_handle = output_embeddings.weight.register_hook(freeze_hook)
print(f"Registered gradient hook to freeze original {actual_original_vocab} output embeddings")
...
# Training arguments - same as Stage 1
...
print("\n" + "=" * 60)
print("Starting Stage 2 Training: New Output Embeddings")
print("=" * 60)
print_memory_usage()
...
stage_info = {
    "stage": 2,
    "name": "stage2_new_output_embeds",
    "description": "Train only NEW output embeddings (lm_head)",
    "trainable_params": trainable_params,
    "total_params": total_params,
    "original_vocab_size": actual_original_vocab,
    "new_vocab_size": actual_new_vocab,
    "previous_stage": STAGE1_MODEL_DIR,
}
...
# Copy token mapping
shutil.copy(f"{STAGE1_MODEL_DIR}/token_mapping.json", f"{OUTPUT_DIR}/token_mapping.json")

print("\n" + "=" * 60)
print("Stage 2 Complete!")
print("=" * 60)
print(f"\nCheckpoint saved to: {OUTPUT_DIR}")
print("Next: Run Stage 3 (train both new embeddings jointly)")
